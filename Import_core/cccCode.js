...
function searchTariff(inputElement, isModal = true) {
    ...
    const results = window.taxData.filter(item => {
        ...
        return cleanedItemCode.startsWith(keyword) ||
            ...
            (item['第一欄稅率'] && item['第一欄稅率'].toLowerCase().includes(keyword)) ||
    ...
    if (results.length > 0) {
        ...
        headers.forEach((header, index) => {
            ...
            } else if (header === '第一欄稅率' || header === '統計數量單位' || header === '稽徵規定' || header === '輸入規定' || header === '輸出規定') {
...
function updateTariff(inputElement, keyword = '') {
    ...
    if (results.length > 0) {
        ...
        if (taxRate && item['第一欄稅率']) {
            const cleanedRate = item['第一欄稅率'].replace(/%/g, '');
            taxRate.value = (certNoVal && certNoItemVal) ? '0' : cleanedRate;
        }
